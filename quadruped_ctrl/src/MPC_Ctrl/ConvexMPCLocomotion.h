...
Vec4<float> footplan;
Vec4<float> footplany;
Vec4<float> footplanz;
...
public:
  ...
  void run(Quadruped<T> &_quadruped, LegController<T> &_legController, StateEstimatorContainer<float> &_stateEstimator,
          DesiredStateCommand<T> &_desiredStateCommand, std::vector<double> gamepadCommand, int gaitType,float x_vel_cmd);
  ...
  Vec3<float> pub_Pf[4];
  Vec3<float> pubFoot[4];
  Vec3<float> Fr_des[4];
  Vec3<float> planfootpoint[4];

  Vec4<float> contact_state;
  bool sswingstate = false;
  bool sstandstate = false;
  Vec3<float> pDes_backup[4];
  Vec3<float> vDes_backup[4];
  Mat3<float> Kp_backup[4];
  Mat3<float> Kd_backup[4];
private:
  void _SetupCommand(StateEstimatorContainer<float> &_stateEstimator, std::vector<double> gamepadCommand,float x_vel_cmd);
  ...
  FootSwingTrajectory<float> pubfootSwingTrajectories[4];
