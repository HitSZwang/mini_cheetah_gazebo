...
ConvexMPCLocomotion::ConvexMPCLocomotion(float _dt, int _iterations_between_mpc)
    : iterationsBetweenMPC(_iterations_between_mpc),  //控制频率用  15
      horizonLength(10),
      dt(_dt),  // 0.002
      trotting(horizonLength*1.5, Vec4<int>(0, 5, 5, 0), 
      Vec4<int>(10, 10, 10, 10), "Trotting"),
  ...
  printf("[Convex MPC]t dt: %.3f iterations: %d, dtMPC: %.3f\n", dt,
...
void ConvexMPCLocomotion::_SetupCommand(
    ...
    std::vector<double> gamepadCommand,float x_cmd) {
  _body_height = 0.32;
  ...
  // x_vel_cmd = 0.2;
  _x_vel_des =
      _x_vel_des * (1 - filter) + x_cmd * filter;  //一阶低通数字滤波
  // _x_vel_des = ;
  // cout<<"x_vel_cmd:"<<_x_vel_des<<endl;
  y_vel_cmd = gamepadCommand[1];
  _yaw_turn_rate = -0.1;
  if (_vectorNavData.com_pos(0, 0) > 0.6 && _vectorNavData.com_pos(0, 0) < 2.7)
  {
    _yaw_turn_rate = -0.18;
  }
  // else if (_vectorNavData.com_pos(0, 0) > 2.0 && _vectorNavData.com_pos(0, 0) < 2.6)
  // {
  //   _yaw_turn_rate = -0.2;
  // }
  ...
  _roll_des = 0.;
  _pitch_des = 0.;//_stateEstimator.getResult().rpy[1];
}
...
void ConvexMPCLocomotion::run(Quadruped<float>& _quadruped,
                              ...
                              int gaitType,float x_vel_cmd) {
  bool omniMode = false;
  // Command Setup
  _SetupCommand(_stateEstimator, gamepadCommand,x_vel_cmd);
  ...
  // 积分达到补偿值*******************************
  // 为了保持在运动过程中身体与地面平行
  if (fabs(v_robot[0]) > .01)  // avoid dividing by zero
  {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    // float tempdata[4];
    // tempdata[i] = _legController.datas[i].p[0];
    // _legController.datas[i].p[0] = 0.03;
    
    pubFoot[i] = _vectorNavData.com_pos +
               seResult.rBody.transpose() *
                   (_quadruped.getHipLocation(i)+ _legController.datas[i].p);
    // _legController.datas[i].p[0] = tempdata[i];
    // pFoot[i] = _legController.datas[i].p;
    // std::cout << "legdata " << i << "腿 x " << _legController.datas[i].p[0] << " y " << _legController.datas[i].p[1] << " z " << _legController.datas[i].p[2] << std::endl;
  }
  ...
  for (int i = 0; i < 4; i++) {
    ...
    footSwingTrajectories[i].setHeight(0.33);
    if(i == 2 || i == 3)
    {
      footSwingTrajectories[i].setHeight(0.22);
    }
  //   if (_vectorNavData.com_pos(0, 0) > 1.6 && _vectorNavData.com_pos(0, 0) < 2.5)
  // {
  //   footSwingTrajectories[i].setHeight(0.38);
  //   if(i == 2 || i == 3)
  //   {
  //     footSwingTrajectories[i].setHeight(0.22);
  //   }
...
//              offset(1)=0.05;
    ...
    Vec3<float> Pff = seResult.position +
                     seResult.rBody.transpose() *
                         (pYawCorrected + des_vel * swingTimeRemaining[i]);
    
    pub_Pf[i] = _vectorNavData.com_pos +
                     seResult.rBody.transpose() * (pYawCorrected + des_vel * swingTimeRemaining[i]);
    // std:: cout<< "pub_PF" << pub_Pf[i][0] << std::endl;
...
/*
    float pub_pfx_rel = _vectorNavData.com_vel(0, 0) * (.5 + 0.0) *
                        stance_time +  //_parameters->cmpc_bonus_swing = 0.0
                    .03f * (_vectorNavData.com_vel(0, 0) - v_des_world[0]) +
                    (0.5f * seResult.position[2] / 9.81f) *
                        (_vectorNavData.com_vel(1, 0) * _yaw_turn_rate);

    float pub_pfy_rel = _vectorNavData.com_vel(1, 0) * .5 * stance_time *3.5+//* dtMPC +
                    .03f * (_vectorNavData.com_vel(1, 0) - v_des_world[1]) +
                    (0.5f * seResult.position[2] / 9.81f) *
                        (_vectorNavData.com_vel(0, 0) * _yaw_turn_rate);
...
*/
    ...
    // }
    //std::cout << "规划落脚点  "<<  i <<  "  "<< Pf[0] << std::endl;
    Pf[2] = -0.00;
    // Pf[1] += pfy_rel;
    planfootpoint[i] = Pf;
    ...
    //Pf[2] = -0.0;
    //pub_Pf[0] += pub_pfx_rel;
    //pub_Pf[1] += pub_pfy_rel;
    // Pf[2] = -0.003;
    //pub_Pf[2] = -0.0;
  ...
  if (_vectorNavData.com_pos(0, 0) > 0.6 && _vectorNavData.com_pos(0, 0) < 2.7)
  {
    pBody_RPY_des[1] = -0.319248;
  }
  // else if (_vectorNavData.com_pos(0, 0) < 2.7 &&_vectorNavData.com_pos(0, 0)  > 2.2)
  // {
  //   pBody_RPY_des[1] =-0.2;
  // }
