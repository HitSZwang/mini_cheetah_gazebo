...
template<typename T>
void LocomotionCtrl<T>::_ContactTaskUpdate(void* input, Quadruped<T> &_quadruped, LegController<T> &_legController, StateEstimatorContainer<T> &_stateEstimator,
                          DesiredStateCommand<T> &_desiredStateCommand, MIT_UserParameters* userParameters){
  _input_data = static_cast<LocomotionCtrlData<T>* >(input);
  //_ParameterSetup(userParameters);
  for(size_t i(0); i<3; ++i){
    ...
    for(size_t j(0); j<4; ++j){
      ...
      ((LinkPosTask<T>*)_foot_task[j])->_Kd[i] = 50.;
    }  
  }
...
template<typename T>
void LocomotionCtrl<T>::_ContactTaskUpdateTEST(void* input, Quadruped<T> &_quadruped, LegController<T> &_legController, StateEstimatorContainer<T> &_stateEstimator,
                          DesiredStateCommand<T> &_desiredStateCommand, MIT_UserParameters* userParameters){
  //(void)data;
  _input_data = static_cast<LocomotionCtrlData<T>* >(input);
  ...
  for(size_t i(0); i<3; ++i){
    ...
    WBCtrl::_Kp_joint[i] = 5;
    WBCtrl::_Kd_joint[i] = 1.5; 
  }
  // Wash out the previous setup
  _CleanUp();
...
/*
template<typename T>
void LocomotionCtrl<T>::_gazebo_PublishData() {
  int iter(0);
  for(size_t leg(0); leg<4; ++leg){
  ...
  for(size_t i(0); i<3; ++i){
   WBCtrl::_wbc_data_lcm.foot_pos[i] = WBCtrl::_model._pGC[linkID::FR][i];
    ...
    for(size_t leg(0); leg<4; ++leg){
      ...
      wbc_commands[leg][i] = WBCtrl::_des_jpos[3*leg + i];
      vwbc_commands[leg][i] = WBCtrl::_des_jvel[3*leg + i];
